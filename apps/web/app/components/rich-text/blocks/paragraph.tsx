...
export const paragraphBlocks: Pick<BlocksStore, "paragraph"> = {
  paragraph: {
    ...
    handleEnterKey(editor) {
       ...
       */
      const [fragmentedNode] = Editor.parent(
        editor,
        editor.selection.anchor.path,
      );
      Transforms.removeNodes(editor);
      const anchorPathPosition = anchorPathInitialPosition[0] || 0;

      // Check if after the current position there is another node
      const hasNextNode = editor.children.length - anchorPathPosition > 1;
      ...
      Transforms.insertNodes(
        editor,
        {
          ...
          children: (isNodeEnd
            ? [{ type: "text", text: "" }]
            : fragmentedNode.children) as DefaultInlineNode[],
        },
        {
          at: hasNextNode ? [anchorPathPosition + 1] : [editor.children.length],
        },
      );
       ...
       */
      Transforms.select(editor, editor.start([anchorPathPosition + 1]));
    },
  },
};
